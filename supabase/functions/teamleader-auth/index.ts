  ...
  try {
    ...
    const tokenResponse = await fetch('https://app.teamleader.eu/oauth2/access_token', {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri,
        client_id: Deno.env.get('VITE_TEAMLEADER_CLIENT_ID') ?? '',
        client_secret: Deno.env.get('VITE_TEAMLEADER_CLIENT_SECRET') ?? ''
      })
    });
    ...
    if (!userInfo.data) throw new Error('Failed to get user data from Teamleader');
    ...
    const hashHex = Array.from(new Uint8Array(hashBuffer))
      ...
      .join('');
    const generatedPassword = `tl_${hashHex.substring(0, 32)}`;
    ...
    if (existingUser) {
      ...
      const { data: authData, error: authError } = await supabase.auth.admin.createUser({
        ...
        user_metadata: {
          ...
          teamleader_id: userInfo.data.id
        }
      });
    ...
    const { error: upsertError } = await supabase.from('teamleader_users').upsert(
      {
        user_id: userId,
        teamleader_id: userInfo.data.id,
